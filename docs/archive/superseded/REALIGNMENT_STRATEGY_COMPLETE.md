...
**Date:** 2026-01-03  
**Status:** Complete Strategy & Planning Document
...
#### 5. docs/DESIGN_PROPOSALS.md (Hardcoded Names)
- **Current Status:** Contains hardcoded baby names and couple names
- **Changes Needed:** Replace with environment variables:
  - Line 168-185: "Jazeel & Michelle" → `VITE_COUPLE_NAMES`
  - Line 168-185: "Baby Maya" → `VITE_BABY_NAME`
  - Line 423-425: Advice capsule form intro text
- **Dependencies:** Privacy standard documentation
- **Blocks:** Privacy compliance, implementation

---

### HIGH - Important Documents for Development

#### 6. docs/DESIGN_PROPOSALS.md (Design System Merge)
...
#### 7. docs/SCHEMA_IMPLEMENTATION.md (Schema Separation)
...
#### 8. docs/MiniMax_Plan/02_PINIA_STATE.md (New)
- **Current Status:** Does not exist
- **Changes Needed:** Create new document with:
...
#### 9. docs/MiniMax_Plan/04_PRIVACY_STANDARD.md (New)
- **Current Status:** Does not exist
- **Changes Needed:** Create new document with:
...
#### 10. docs/MiniMax_Plan/05_VISUAL_THEME.md (New)
- **Current Status:** Does not exist
- **Changes Needed:** Create new document with:
...
#### 11. docs/architecture/02_TARGET_ARCHITECTURE.md (New)
- **Current Status:** Does not exist
- **Changes Needed:** Create new document with:
...
#### 12. docs/DESIGN_PROPOSALS.md (Implementation Roadmap)
...
#### 13. docs/INTEGRATION_PATTERNS.md (AI Section Updates)
...
#### 14. docs/MiniMax_Plan/07_ASSET_MANIFEST.md (New)
- **Current Status:** Does not exist
- **Changes Needed:** Create new document with:
...
#### 15. docs/RESEARCH_VERCEL.md (Environment Variables)
...
#### 16. docs/DESIGN_PROPOSALS.md (Visual Design System)
...
#### 17. Historical Document Archive
...
1. Run `20250102_data_migration.sql` migration script
2. Transform legacy JSON data into normalized tables:
   - Parse `activity_data` JSON for each submission type
   - Map to appropriate table columns with type casting
   - Preserve original `created_at` timestamps
   - Store legacy IDs for rollback reference
3. Verify data integrity:
   - Counts match between source and destination
   - Sample data validation across all activity types
   - Foreign key relationships established correctly

**Phase 4: Cutover & Legacy Cleanup (Week 4)**
1. **Switch reads to new schema:**
   - Update frontend queries to use `baby_shower.*` tables
   - Modify realtime subscriptions to new table names
   - Update Edge Functions to stop dual-write
2. **Monitoring period:** 48-hour observation for issues
3. **Legacy cleanup:** Drop `public.submissions` table (with backup)
4. **Documentation update:** Mark legacy schema as deprecated

#### Firewall Pattern Implementation

The MiniMax vision introduces a security "firewall" pattern with strict schema separation:

```sql
-- public schema: Insert-only for anonymous users
CREATE SCHEMA public;

-- public.guestbook_entries (RLS: INSERT only)
-- public.votes (RLS: INSERT only)

-- internal schema: No public access, Edge Functions only
CREATE SCHEMA internal;

-- internal.event_archive (No RLS, service role only)
-- internal.user_metadata (No RLS, service role only)

-- RLS Policy Examples:
ALTER TABLE public.guestbook_entries ENABLE ROW LEVEL SECURITY;
ALTER TABLE internal.event_archive FORCE ROW LEVEL